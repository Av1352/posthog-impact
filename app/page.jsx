...
function generateWhy(engineer) {
    ...
    if (strongest.key === 'complexity_score') {
        parts.push(`Ships substantially complex work — avg ${engineer.avg_pr_size.toLocaleString()} lines across ${engineer.avg_files} files per PR.`)
    ...
    if (strongest.key !== 'complexity_score') {
        parts.push(`Avg PR size ${engineer.avg_pr_size} lines — scores higher on consistency than raw complexity.`)
    }
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: '24px 32px', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '20px' }}>
                <div>
                    ...
                    <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px', fontFamily: sans }}>
                        Impact = complexity of work owned + consistency of delivery + code stability. Not commits, lines of code, or raw PR count.
                    </p>
                </div>
                {data && (
                ...
                <>
                    ...
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px 16px' }}>
                        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                            ...
                            {[
                                { color: '#a78bfa', label: 'Complexity (35%)', desc: 'Avg PR size × files/PR. Engineers taking on harder, larger work score higher.' },
                                { color: '#3fb950', label: 'Consistency (30%)', desc: 'PR throughput across active weeks. Sustained contribution beats a single big sprint.' },
                                { color: '#58a6ff', label: 'Cadence (20%)', desc: 'Contribution spread across the 90-day window. Engineers active across more weeks score higher than those with the same output in a short burst.' },
                                { color: '#f0883e', label: 'Longevity (15%)', desc: 'PRs without hotfix/revert/rollback patterns. Code that stands without immediate fixes.' }
                            ].map(m => (
                        ...
                        <div style={{ fontSize: '10px', color: 'var(--text-dim)', marginTop: '10px', fontFamily: sans, borderTop: '1px solid var(--border)', paddingTop: '8px' }}>
                            Each dimension normalized 0–1 across engineers with ≥3 PRs. Scope note: review body content was excluded as a deliberate time constraint — bulk PR metadata gives sufficient signal for this analysis at 90-minute scope. Not measured: commits, raw line count, PR count in isolation.
                        </div>
                    </div>
                </>
            )}
